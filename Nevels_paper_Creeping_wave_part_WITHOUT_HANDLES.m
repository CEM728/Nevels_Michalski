...
%%
kxx = linspace(0*k_air,1e4*k_air,1e4*len);
kxy = linspace(-1e3*k_air,1e1*k_air,1e4*len);    
...
c2_start_imag = kxy(c1_eny_loc + 1);
...
c3_start_imag = kxy(c1_eny_loc - 1);
%
...
for i = 1 : length (x)
    ...
    G_c1 = 1./D_c1;
    integrand_1 = G_c1.*exp(-1i*kz1_c1*x(i));
    ...
    G_c2 = 1./D_c2;
    integrand_2 = G_c2.*exp(-1i*kz1_c2*x(i));
    ...
    G_c3 = 1./D_c3;
    integrand_3 = G_c3.*exp(-1i*kz1_c3*x(i));
    H_c3(i) = sum(integrand_3);
    
end
H = H_c1 +  H_c3; % C2 Contour must be zero anyway, therefore taken out
%% Plot Figure
