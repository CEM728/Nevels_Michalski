...
c1_start_imag = -kxx(bp1_loc - 1)*1e2;
...
for i = 1 : length (x)
    ...
    for j = 1 : length(kx_c1)
        ...
        if abs(imag(kx_c1(j))) < abs(Silver_branch_cut_loc_c1)
            % Enfore bottom sheet on k1 and k2
            % Im(kz1) > 0, Re(kz1) > 0
            % Im(kz2) > 0, Re(kz2) > 0
            if real(kz1_c1(j)) < 0
    ...
    for j = 1 : length(kx_c3)
        ...
        if abs(imag(kx_c1)) > abs(Silver_branch_cut_loc_c3)
...
cleanfigure();
matlab2tikz('filename',sprintf('nevels_michalski_decay_plot_rev4.tex'))
%%
save nevels_michalski_creeping_wave_rev4.mat % Save data to plot the branch cuts
save('test_rev4.mat', 'H', 'x')              % Save H and x for plotting along SPP
