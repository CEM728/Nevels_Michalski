...
for i = 1 : length (x)
    ...
    for j = 1 : length(kx_c1)
        ...
        if imag(kx_c1) < Silver_branch_cut_loc_c1
            ...
            % This is bottom sheet of kz1, but top sheet of kz2
            % Im(kz1) > 0, Re(kz1) > 0
            % Im(kz2) < 0, Re(kz2) < 0
            if real(kz1_c1(j)) < 0
                kz1_c1(j) = -real(kz1_c1(j)) + 1i*imag(kz1_c1(j));
...
%%
save nevels_michalski_creeping_wave_2500nm.mat % Save data to plot the branch cuts
save('test_2500nm.mat', 'H', 'x')              % Save H and x for plotting along SPP
